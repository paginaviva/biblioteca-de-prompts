...
export async function PUT(
  ...
  try {
    ...
    if (data.slug) {
      ...
      const slugTaken = await prisma.language.findUnique({
    ...
    return NextResponse.json({ data: language, success: true })
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    // prompts (they keep their language string unchanged).
    await prisma.language.delete({
      where: { id: params.id },
    })

    return NextResponse.json({
      data: { message: t("languageDeleted") },
      success: true,
    })
  } catch (error) {
    console.error("Error deleting language:", error)
    return NextResponse.json(
      { error: t("failedToDeleteLanguage") },
      { status: 500 }
    )
  }
}
