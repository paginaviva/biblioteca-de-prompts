...
export async function PUT(
  ...
  try {
    ...
    const data = updateClientProjectSchema.parse(body)
    ...
    if (!existing) {
      return NextResponse.json(
        { error: t("clientProjectNotFound") },
        { status: 404 }
      )
    }

    // Slug uniqueness: another client/project must not hold the slug.
    if (data.slug) {
      const normalizedSlug = slugify(data.slug)
      const slugTaken = await prisma.clientProject.findUnique({
    ...
    return NextResponse.json({ data: clientProject, success: true })
  } catch (error) {
    ...
    return NextResponse.json(
      { error: t("failedToUpdateClientProject") },
      { status: 500 }
    )
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    // N:M junction rows (PromptClientProject) are deleted by the existing
    // onDelete: Cascade from this side, so prompts simply lose this
    // client/project while keeping all their other relations.
    await prisma.clientProject.delete({
      where: { id: params.id },
    })

    return NextResponse.json({
      data: { message: t("clientProjectDeleted") },
      success: true,
    })
  } catch (error) {
    console.error("Error deleting client-project:", error)
    return NextResponse.json(
      { error: t("failedToDeleteClientProject") },
      { status: 500 }
    )
  }
}
